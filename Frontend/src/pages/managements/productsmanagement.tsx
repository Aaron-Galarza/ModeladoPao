...
export interface Product {
  ...
  data: {
    ...
    descripcion: string;
    categoria: string;
    imagenURL: string; // <-- CAMBIADO AQUÍ
    isActive?: boolean;
  };
}
...
const ProductsManagement: React.FC = () => {
  ...
  const extractCategories = (productsList: Product[]): string[] => {
    const allCategories = productsList
      .map(product => product.data.categoria)
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(productosCollection, (snapshot) => {
      ...
      setCategories(extractCategories(productosList));
      setLoading(false);
    });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const productData = {
        ...
        imagenURL: formData.imagenURL, // <-- CAMBIADO AQUÍ
  ...
  const handleEdit = (product: Product) => {
    ...
    setFormData({
      ...
      imagenURL: product.data.imagenURL, // <-- CAMBIADO AQUÍ
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {products.map(product => (
                <div key={product.id} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                  {product.data.imagenURL && ( // <-- CAMBIADO AQUÍ
                    <img
                      src={product.data.imagenURL} // <-- CAMBIADO AQUÍ
